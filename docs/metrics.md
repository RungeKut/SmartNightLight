...
Счётчики считают события, а не измерения: событие — это шум, отделённый
от предыдущего паузой в 30 секунд. Непрерывный разговор в комнате даёт
одну точку, а не десятки. Подробности — в `docs/modules/sound.md`.

Оба счётчика живут с момента загрузки и обнуляются при перезагрузке. Для
типа `counter` это штатно: `increase()` и `rate()` сброс учитывают.
